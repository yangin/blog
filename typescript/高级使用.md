...
类型保护是 Typescript 通过一些`特定表达式`来细化类型的一种机制。通过类型保护，我们可以在特定的区块中保证变量属于某种确定的类型，以规避不合法的操作，提高代码质量。
...
类型收窄是指在通过类型断言或者类型保护来减少类型推断范围的这一过程。从而更精准的推断出变量的类型。
...
1. never 类型表示从未观察到的值。在返回类型中，这意味着函数抛出异常或终止程序的执行。
...
返回类型为 void 的上下文类型不会强制函数不返回某些内容, 只是会被忽略。
...
infer 关键字用于推断出类型。infer 后接一个新的泛型变量。
